...
// ============ PAINEL DE MENSAGENS ============
function MessagesPanel({ childId, childName }: { childId: string; childName: string }) {
  ...
  if (contacts.length === 0) {
    return (
      <div className="rounded-2xl bg-white p-8 text-center shadow-lg">
        ...
        <p className="mt-2 font-bold text-slate-600">Nenhum professor vinculado a {childName}</p>
        <p className="mt-1 text-sm text-slate-400">
          Quando um professor estiver vinculado à criança, vocês poderão trocar mensagens aqui.
        </p>
      </div>
    )
  }
    ...
    <div className="rounded-2xl bg-white p-4 shadow-lg">
      <div className="mb-3 flex items-center gap-2 overflow-x-auto">
        {contacts.map((contact) => (
          ...
          >
            {contact.name} · {contact.role === "teacher" ? "Professor(a)" : "Terapeuta"}
          </button>
        ))}
      </div>
            ...
            <div key={message.id} className={`flex ${message.mine ? "justify-end" : "justify-start"}`}>
              <div
                className={`max-w-[80%] rounded-2xl px-3 py-2 text-sm ${
                  message.mine ? "bg-purple-500 text-white" : "bg-white text-slate-700 shadow"
                }`}
              >
      ...
      <form onSubmit={handleSend} className="mt-3 flex gap-2">
        <input
          ...
          className="flex-1 rounded-xl border-2 border-slate-200 bg-slate-50 px-3 py-2 text-sm font-medium text-slate-700 focus:border-purple-400 focus:outline-none"
        />
        <button
          type="submit"
          disabled={isSending}
          className="flex items-center justify-center rounded-xl bg-purple-500 px-4 text-white shadow disabled:opacity-50"
        >
